...
class AccountInvoice(models.Model):
    ...
    def _amount_all_currency(self):
        for inv in self:
            ...
            print(amount_tax_currency)
            amount_total_currency = amount_untaxed_currency + amount_tax_currency
            sign = inv.type in ['in_refund', 'out_refund'] and -1 or 1
            inv.amount_untaxed_currency = inv.convert_to_devise(amount_untaxed_currency * sign, inv.currency_id, inv.company_id.currency_id)
            inv.amount_tax_currency = inv.convert_to_devise(amount_tax_currency * sign, inv.currency_id, inv.company_id.currency_id)
            inv.amount_total_currency = inv.convert_to_devise(amount_total_currency * sign, inv.currency_id, inv.company_id.currency_id)
...
class AccountInvoiceLine(models.Model):
    ...
    dossier_id = fields.Many2one('neurones.dossier.manager', "Dossier commercial", related="move_id.dossier_id", required=False)
